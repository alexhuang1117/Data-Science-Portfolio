...
Bootstrapping of powerlaw distribution coeffcients. Bootstraping is sampling the data with replacement to form multiple data samples. Each sample will return a parameter for $\alpha$ and $c$. This part takes some time to run.
```{r}
...
```
bs_p$p shows the p value of the distribution being a power-law distribution. High p value means the disitribution is likely to follow power-law.

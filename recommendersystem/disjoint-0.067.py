...
def set_articles(articles):
    ...
    global theta_hat
    theta_hat = {x:np.matmul(inv_Aa.get(x),b_a.get(x)) for x in articles}
...
def update(reward):
	...
	reward =0.35030347412653362
    else:
	reward =3.1622776601683795

    #print reward
    global A_0, b_0, A_a, z, x, B_a, b_a

    A_a[chosen] = A_a.get(chosen) + np.matmul(x,x.T)

    b_a[chosen] = b_a.get(chosen) + reward*x

    inv_Aa[chosen] = inv(A_a.get(chosen))

    theta_hat[chosen] = np.matmul(inv_Aa.get(chosen),b_a.get(chosen))
    return 0
